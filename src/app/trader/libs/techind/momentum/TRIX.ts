...
export class TRIX extends Indicator {
  constructor(input: TRIXInput) {
    ...
    let ema              = new EMA({ period : period, values : [], format : (v) => v});
    let emaOfema         = new EMA({ period : period, values : [], format : (v) => v});
    let emaOfemaOfema    = new EMA({ period : period, values : [], format : (v) => v});
    let trixROC          = new ROC({ period : 1, values : [], format : (v) => v});
