...
export class KST extends Indicator { 
  constructor(input: KSTInput) {
    ...
    let sma1        = new SMA({ period : smaPer1, values: [], format : (v) => v});
    let sma2        = new SMA({ period : smaPer2, values: [], format : (v) => v});
    let sma3        = new SMA({ period : smaPer3, values: [], format : (v) => v});
    let sma4        = new SMA({ period : smaPer4, values: [], format : (v) => v});
    let signalSMA   = new SMA({ period : signalPeriod, values: [], format : (v) => v});
    ...
    this.generator = (function* () {
      ...
      while (true) {
        ...
        if (index < firstResult) {
          ...
          kst1 = (RCMA1 * 1) + (RCMA2 * 2) + (RCMA3 * 3) + (RCMA4 * 4);
        }
        signal = (kst1 !== undefined) ? signalSMA.nextValue(kst1) : undefined;
