...
export class Base {
  ...
  asyncIndicatorRunner;// = new AsyncIndicatorRunner();
  ...
  tick(candle, done) {
    ...
    calculateSyncIndicators(candle, done) {
      ...
      _.each(this.indicators, function(i: any) {
        if(i.input === 'price')
          i.update(price);
        if(i.input === 'candle')
          i.update(candle);
  ...
  propogateTick(candle) {
    ...
    if(!this.completedWarmup) {
      ...
      if(this.mode === 'realtime') {
        const startTimeMinusCandleSize = this.startTime
          .clone()
    ...
    if(this.completedWarmup) {
     // this.log(candle);
      ...
      if(
        ...
        return this.tick(this.deferredTicks.shift(), null)
      }
    }

    const indicators = {};
    _.each(this.indicators, (indicator: any, name) => {
