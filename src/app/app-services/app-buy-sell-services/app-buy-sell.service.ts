...
export class AppBuySellService {
  ...
  constructor(
   ...
   this.isSellRunning$().subscribe(isRunning =>{
      ...
      if(isRunning){
        if(!this.sellInterval) this.sellInterval = setInterval(()=> this.runSellBots(), 60 * 1000);
      }else {
  ...
  async startSellBots(){
    await this.storage.upsert('isSellRunning', true);
    this._isSellRunning.next(true);
    this.runSellBots();
  }

  async stopSellBots(){
    await this.storage.upsert('isSellRunning', false);
