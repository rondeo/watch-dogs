      ...
      //  else if (action === 'BUY')obs = currentAPI.buyLimit(base, coin, amountCoin, rate );
        ...
        obs.subscribe((res: VOOrder) => {
          ...
          if (res && res.uuid) {
            ...
            snackBar.open('Order Set! ' + msg, 'x', {panelClass: 'alert-green', duration: 2000});
            resolve(order);
            // resolve(order);

          } else {
            reject({message: 'Error ' + res.message});
            snackBar.open('Error ' + res.message, 'x', {panelClass: 'alert-red', duration: 3000});
          }

        }, error => {
          reject(error);
          snackBar.open('Error ' + error.message, 'x', {panelClass: 'alert-red'});
        });
